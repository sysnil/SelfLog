...
def evaluate(groundtruth, parsedresult):
    ...
    (precision, recall, f_measure, accuracy, PTA, RTA) = get_accuracy(df_groundtruth['EventId'], df_parsedlog['EventId']) #df_parsedlog['EventId']
    print('Precision: %.4f, Recall: %.4f, F1_measure: %.4f, Parsing_Accuracy: %.4f, PTA: %.4f, RTA: %.4f'%(precision, recall, f_measure, accuracy, PTA, RTA))
    return f_measure, accuracy, PTA, RTA
...
def get_accuracy(series_groundtruth, series_parsedlog, debug=True):
    ...
    PTA = float(crrurate_template) / len(series_parsedlog_valuecounts)
    RTA = float(crrurate_template) / len(series_groundtruth_valuecounts)
    return precision, recall, f_measure, accuracy, PTA, RTA